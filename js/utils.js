...
function renderTasks(tasks) {
	...
	tasks
		...
		.forEach((value, i) => {
			...
			const item = `
				<div
					class="task ${done ? 'done' : ''} ${pinned ? 'pinned' : ''}"
					data-task-id='${id}'
					draggable='true'
				>
					<p class='task-text'>${task}</p>
					<span class="task-index ${done ? 'none' : ''}">${i + 1}</span>
					<div class='task-btns'>
						<button
							class="task-done ${done ? 'active' : ''}"
						>
							${doneSvg}
						</button>
						<button
							class="task-pinned ${pinned ? 'active' : ''}"
						>
							${pinnedSvg}
						</button>
						<button class='task-edit'>${editSvg}</button>
						<button class='task-delete'>${delSvg}</button>
					</div>
				</div>
			`;
